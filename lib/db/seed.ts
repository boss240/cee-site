...
async function main() {
  ...
  if (existingAdmin.length === 0) {
    ...
    console.log(`✓ Створено адміна: ${adminEmail} (пароль зі змінної SEED_ADMIN_PASSWORD)`);
  } else {
    console.log(`• Адмін ${adminEmail} вже існує, пропускаю`);
  }
  ...
  if (existingModels.length === 0) {
    ...
    console.log("✓ Додано 2 дефолтні AI-моделі (основна + резервна)");
  } else {
    console.log("• AI-моделі вже існують, пропускаю");
  }

  const existingCompany = await db.select().from(schema.companyProfile);
  if (existingCompany.length === 0) {
    await db.insert(schema.companyProfile).values({});
    console.log("✓ Створено профіль компанії за замовчуванням");
  }
  ...
  if (existingGateways.length === 0) {
    ...
    console.log("✓ Додано 4 платіжні шлюзи (не підключені)");
  }
  ...
  if (existingServices.length === 0) {
    ...
    console.log("✓ Додано 2 приклади послуг");
  }
  ...
  if (existingPosts.length === 0) {
    ...
    console.log(`✓ Додано ${SEED_POSTS.length} статей блогу`);
  } else {
    console.log("• Статті блогу вже є, пропускаю");
  }
  ...
  if (existingDocs.length === 0) {
    ...
    console.log(`✓ Додано ${SEED_DOCUMENTS.length} документів до бази знань`);
  } else {
    console.log("• Документи бази знань уже є, пропускаю");
  }
  ...
  if (existingSources.length === 0) {
    ...
    console.log(`✓ Додано ${SEED_SOURCES.length} джерел новин (вимкнені до перевірки)`);
  } else {
    console.log("• Джерела новин уже є, пропускаю");
  }
  ...
  if (existingDigests.length === 0) {
    ...
    console.log("✓ Додано вступний дайджест");
  } else {
    console.log("• Дайджести вже є, пропускаю");
  }
