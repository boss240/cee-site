...
import { Home, DollarSign, Sun, Battery, CheckCircle2, ChevronRight } from "lucide-react";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations("Placeholder.citizens");
  return { title: t("title"), description: t("hint"), alternates: await alternatesFor("/citizens") };
}
...
export default async function CitizensPage({ params }: Props) {
  ...
  const solutions = t.raw("solutions") as Item[];
  ...
  return (
    <>
      ...
      <section className="border-b border-[var(--color-line)] bg-[var(--color-surface)]">
        <div className="mx-auto max-w-6xl px-4 py-20">
          ...
          <ul className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {whyItems.map((item, i) => {
              const icons = [Sun, Battery, DollarSign];
      ...
      <section className="mx-auto max-w-6xl px-4 py-20">
        <Reveal>
          <p className="mono-label text-[var(--color-brand-text)]">{t("solutionsLabel")}</p>
          <h2 className="mt-2">{t("solutionsTitle")}</h2>
          <p className="mt-4 max-w-2xl text-[var(--color-fg-muted)]">
            {t("solutionsText")}
          </p>
        </Reveal>
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2">
          {solutions.map((sol, i) => (
            <Reveal as="div" key={sol.title} delay={i * 90}>
              <Link
                href="/contacts"
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-[var(--color-fg-muted)]">{sol.text}</p>
                  </div>
      ...
      <section className="border-b border-[var(--color-line)] bg-[var(--color-surface)]">
        <div className="mx-auto max-w-6xl px-4 py-20">
          <Reveal>
            <p className="mono-label text-[var(--color-brand-text)]">{t("stepsLabel")}</p>
            <h2 className="mt-2">{t("stepsTitle")}</h2>
            <p className="mt-4 max-w-2xl text-[var(--color-fg-muted)]">
              {t("stepsText")}
            </p>
          </Reveal>
            ...
            {steps.map((step, i) => (
              <Reveal as="li" key={step.title} delay={i * 60}>
                <div className="flex gap-4">
                  <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-[var(--color-brand)] text-sm font-semibold text-white">
                    {i + 1}
                  </div>
                  <div className="flex-1">
                    <h3 className="font-semibold">{step.title}</h3>
                    <p className="mt-1 text-sm text-[var(--color-fg-muted)]">{step.text}</p>
                  </div>
                </div>
              </Reveal>
            ))}
